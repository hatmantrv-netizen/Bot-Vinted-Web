...
# =============================================================================
# DB SHIM — Turso via API HTTP (aiohttp, zéro dépendance supplémentaire)
# =============================================================================

class _DBIntegrityError(Exception):
    """Exception commune aux deux backends (aiosqlite + Turso)."""
    pass


def _to_turso_val(v):
    """Convertit une valeur Python au format Turso HTTP API."""
    if v is None:
        return {"type": "null"}
...
def _from_turso_val(cell):
    """Convertit une valeur Turso HTTP en valeur Python."""
...
class _TursoResultSet:
    """Résultat compatible curseur aiosqlite."""
    def __init__(self, result: dict):
        raw = result.get("rows", [])
        self._rows = [tuple(_from_turso_val(c) for c in row) for row in raw]
...
class _TursoExecProxy:
    """Awaitable ET async context manager pour db.execute(...)."""
...
class TursoDB:
    """
    Client Turso via API HTTP — utilise aiohttp (déjà dans requirements).
    Interface 100% compatible aiosqlite, aucune dépendance supplémentaire.
    """
    ...
    async def _pipeline(self, stmts: list) -> list:
        """Envoie plusieurs statements en un seul appel HTTP."""
        requests = []
        for s in stmts:
            if isinstance(s, str):
                requests.append({"type": "execute", "stmt": {"sql": s, "args": []}})
            else:
                sql, args = s[0], s[1]
                requests.append({
                    "type": "execute",
                    "stmt": {"sql": sql, "args": [_to_turso_val(a) for a in args]}
                })
        requests.append({"type": "close"})

        sess = await self._sess()
        async with sess.post(self._url, json={"requests": requests}, headers=self._headers) as resp:
        ...
        for item in data.get("results", []):
            if item.get("type") == "error":
                ...
                if "UNIQUE" in (msg + code).upper() or "CONSTRAINT" in (msg + code).upper():
                    raise _DBIntegrityError(msg)
                raise Exception(f"Turso: {msg}")
            if item.get("type") == "ok" and item.get("response", {}).get("type") == "execute":
                results.append(item["response"]["result"])
        return results

    def execute(self, sql: str, params=()):
        async def _do():
            res = await self._pipeline([(sql, list(params))])
            return _TursoResultSet(res[0] if res else {})
        return _TursoExecProxy(_do())

    async def executemany(self, sql: str, param_list):
        stmts = [(sql, list(p)) for p in param_list]
        if stmts:
            await self._pipeline(stmts)
    ...
    async def commit(self):   pass  # Turso = auto-commit
    async def rollback(self): pass  # no-op
...
async def lifespan(app: FastAPI):
    if USE_TURSO:
        db = TursoDB(url=TURSO_URL, token=TURSO_TOKEN)
        logging.info("🌐 Base de données : Turso (SQLite distant persistant)")
    else:
        ...
        logging.info("🗄️ Base de données : SQLite local")
        ...
        for i in range(payload.count):
            ...
            while attempts < 10:
                ...
                try:
                    ...
                    codes.append(code)
                    break
                except (_DBIntegrityError, aiosqlite.IntegrityError):
